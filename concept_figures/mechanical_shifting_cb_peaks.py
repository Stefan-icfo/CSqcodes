...
# --- Second sinusoidal line (higher frequency, from the left) ---
# 30% higher frequency; runs from left axis up to V_start where red begins
sine_freq2  = sine_freq * 1.8
V_sine2 = V[V <= V_start]
# Phase chosen so the curve arrives at sine_center at V_start (matching red's start)
G_sine2 = sine_center + sine_amp * np.sin(2 * np.pi * sine_freq2 * (V_sine2 - V_start))
...
#ax.plot(V_sine2, G_sine2, color='royalblue', lw=1.5, label=r'V_sig (high $f$)')
#ax.plot(V_sine2, G_sine2, color='green', lw=1.5, label=r'V_probe ($\omega_m+\omega_{RLC}$)')
#ax.plot(V_sine, G_sine, color='red', lw=1.5, label=r'I_sig ($\omega_{RLC}$)')
