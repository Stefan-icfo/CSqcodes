...
AVG_BLOCK = 3            # non-overlapping block size

# ---- Peak integration window (MHz) ----
PEAK_START_MHZ = 153.66
PEAK_END_MHZ   = 153.664

# ---- Baseline removal inside the window ('none' | 'constant' | 'linear') ----
BASELINE_MODE = 'none'
BASELINE_EDGE_FRACTION = 0.1  # use 10% edges for baseline estimation
...
# averaged curve for visibility (punti neri + linea)
if x_avg.size and y_avg.size:
    ax.plot(x_avg, y_avg, '-o', ms=3.5, lw=1.8, label=f"{AVG_BLOCK}-point average")

# --- RED FILL: area integrata dei dati RAW ---
# Usa esattamente i punti usati per l'integrazione RAW (dopo baseline opzionale).
if FILL_RAW_RED:
    ...
    # Clip a zero solo per l'estetica del fill (l'integrazione sopra NON clippa)
    y_fill_raw_pW = np.clip(y_fill_raw_pW, 0.0, None)
    ax.fill_between(x_fill_raw, 0.0, y_fill_raw_pW, color='crimson', alpha=0.35,
                    label='Integrated area (RAW, red)')

# --- GREEN FILL: area integrata dei dati AVERAGED ---
# Anche qui usiamo gli stessi punti averaged usati per l'integrazione (dopo baseline).
...
ax.set_title("Noise-subtracted PSD: integrated areas (RAW red, AVG green)", fontsize=18, fontname="Calibri")

ax.tick_params(axis='both', which='major', labelsize=16)
for lab in (ax.get_xticklabels() + ax.get_yticklabels()):
    lab.set_fontname("Calibri")

# Annotazione valori aree
...
out_png = os.path.join(out_dir, f"psd_area_raw_red_avg_green_run{run_data}_minus_{run_background}_avg{AVG_BLOCK}.png")
