...
UNITS_IN_VOLTS = True  # True if x is stored in V; False if already in µV

# where to START the fit (in µV) by searching backwards from the edge
START_BAND_UV = (-1400.0, -1000.0)  # your request: between -1.45 and -1.2 mV

# window around edge: end later so plateau lasts longer in the zoom plot
W_PRE_US  = 8.0     # how far back we are allowed to search (and include) before edge
W_POST_US = 60.0    # how long after edge we include (e.g. edge at 400 us -> end 460 us)

MIN_POINTS_FIT = 6
MIN_R2 = 0.30       # with many plateau points, R2 can vary; keep loose and inspect

PLOT_DEBUG_EDGES = True
PLOT_ONE_ZOOM = True
ZOOM_EDGE_INDEX = 10  # which edge to zoom (0..N-1)
# =========================================================
...
def choose_start_index_by_band(t, v, edge_idx, dt, w_pre_us, band_uv, units_in_volts=True):
    """
    Search backwards from edge_idx within W_PRE_US and pick the earliest sample
    that falls inside band_uv (in µV). If none found, fallback to edge_idx - pre.
    """
    scale = 1e6 if units_in_volts else 1.0  # V -> µV
    lo, hi = band_uv
    ...
    if inside.size > 0:
        # earliest index within search region that is inside the band
        start_rel = int(inside[0])
        return i0 + start_rel

    # fallback: just take beginning of the pre-window
    return i0


def fit_one_edge_with_custom_start(t, v, edge_idx, dt, i_start, w_post_us):
    """
    Fit from i_start to edge_idx+post, with time zero at i_start.
    """
    ...
    # initial guesses: C0 from early points (first 3-5), plateau from last points
    n0 = min(5, len(vv))
...
def main():
    ...
    for k, eidx in enumerate(edges):
        ...
        print(f"edge {k:02d}: start@{t[i_start]*1e6:.2f} µs, end@{t[res['i_end']-1]*1e6:.2f} µs | "
              f"tau={tau*1e6:.3f} µs | R2={r2:.3f} | N={res['i_end']-res['i_start']}")

        fit_results.append((k, eidx, res))
        ...
        t_abs = (t[i_start] + res["tt"]) * 1e6
        plt.plot(t_abs, res["vfit"] * 1e6, lw=2)

    plt.xlabel("time (µs)")
    plt.ylabel("x (µV)")
    plt.title(f"Custom-start exp-rise fits | kept {len(taus)} (R2>{MIN_R2})")
    plt.tight_layout()
    plt.show()

    if PLOT_ONE_ZOOM and fit_results:
        idx = min(ZOOM_EDGE_INDEX, len(fit_results)-1)
        k, eidx, res = fit_results[idx]
        ...
        plt.xlabel("time (µs)")
        plt.ylabel("x (µV)")
        plt.title(f"Zoom edge {k:02d}: from {t[res['i_start']]*1e6:.1f} µs to {t[res['i_end']-1]*1e6:.1f} µs")
        plt.legend()
        plt.tight_layout()
        plt.show()

    if taus:
        taus_us = np.array(taus) * 1e6
        print("\n=== SUMMARY (kept fits) ===")
        print(f"Mean tau = {taus_us.mean():.3f} µs")
        print(f"Std  tau = {taus_us.std():.3f} µs")
        print(f"N fits   = {len(taus_us)}")
        print(f"Median R2 = {np.median(r2s):.3f}")
    else:
        print("\nNo fits kept. Try lowering MIN_R2 or increasing W_POST_US.")
