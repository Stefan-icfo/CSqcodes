...
run_ids = [151,152]#list(range(443, 492, 2))  # run_ids from 240 to 282 with a step of 2
lags=1000
...
def fitting_func(x_ms, a, b, c, d, e):
    """
    Underdamped ACF:
    R(t) = a * exp(-b*|t|) * [cos(c*|t|) + e * sin(c*|t|)] + d
    where:
      a = A (amplitude)
      b = Gamma_per_ms (ms^-1)
      c = omega_d_per_ms (rad/ms) = 2*pi*f_kHz
      d = C (offset)
      e = mix (~ Gamma/omega_d), dimensionless
    """
    u = np.abs(x_ms)
    env = np.exp(-b * u)
    return a * env * (np.cos(c * u) + e * np.sin(c * u)) + d
