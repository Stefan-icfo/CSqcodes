...
def fitting_func(x, a, b, c, d, e,beta):
    return a * np.exp(-(b * x)**beta) + c + d * np.exp(-e * x)
...
for run_id in run_ids:
    try:
        ...
        lower_bounds = [1e-10, -np.inf, -np.inf, 1e-10, -np.inf,1e-10]  # a > 0, d > 0
        upper_bounds = [np.inf, np.inf, np.inf, np.inf, np.inf,1]    # no upper limits

        popt, pcov = curve_fit(fitting_func, xData, yData, 
                       p0=[a0, b0, c0, d0, e0,beta0],
