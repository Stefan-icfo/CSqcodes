...
qc.config["core"]["db_location"]='.\\Data\\Raw_data\\CD12_B5_F4v3.db'#for meas with other sideband#for meas with lower SL drive# measurement of 4.8.
background_id=95


freq,V_background=extract_1d(background_id, data_1d_name = data_name, setpoint_name = setpoint_name,  plot = True)
print(f"nr pt background run {len(V_background)}")

qc.config["core"]["db_location"]='.\\Data\\Raw_data\\CD12_B5_F4v6.db'

run_ids= list(range(4, 21, 2))
...
print(f"nr pt full run {len(test_spectrum)}")
...
for i, run_id in enumerate(run_ids):
   ...
   #V_spectrum=V_spectrum-V_background suspend background substraction for now
   ...
   spectra[:, i] = p_spectrum#/probe_amp**2#scale it py the probe amp squared
...
plt.title("square-scaled probe amp spectrum v7 4-21")
plt.title("probe amp spectrum v7 4-21")
