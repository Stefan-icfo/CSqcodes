...
# ─── g vs Ne and linewidth vs Ne as two separate figures ─────────────────────

# Figure A: g/2π vs Ne
fig_g, ax_g = plt.subplots()
ax_g.errorbar(electron_nrs2_plot, g2_Hz / 1e6, yerr=g_errors2_Hz / 1e6,
              fmt='g*', capsize=5, label='untensioned (140 MHz)')
ax_g.errorbar(e_nrs_manual, g_manual_Hz / 1e6, yerr=g_errors_manual_Hz / 1e6,
              fmt='g*', capsize=5, label='tensioned')
ax_g.plot(x1_line, slope_g * x1_line / 1e6, 'g-', linewidth=2)
ax_g.set_xlabel("nr electrons")
ax_g.set_ylabel("g/2\u03c0 (MHz)", color='g')
ax_g.tick_params(axis='y', labelcolor='g')
...
# Figure B: linewidth vs Ne
fig_lw, ax_lw = plt.subplots()
ax_lw.plot(electron_nrs2_plot, np.array(linewidths2l_plot) / 1e3, 'b^')
ax_lw.set_xlabel("nr electrons")
ax_lw.set_ylabel("linewidth (kHz)", color='b')
ax_lw.tick_params(axis='y', labelcolor='b')
ax_lw.set_xlim(left=0)
ax_lw.set_ylim(bottom=0)
ax_lw.xaxis.set_major_locator(MaxNLocator(integer=True))
plt.title("linewidth vs electron nr")
plt.tight_layout()
plt.show()

# ─── g and linewidth stacked, shared x-axis ──────────────────────────────────

fig, (ax_g, ax_lw) = plt.subplots(2, 1, sharex=True, figsize=(7, 6))
...
ax_g.set_xlim(left=0)
ax_g.set_ylim(bottom=0)
ax_g.yaxis.set_major_locator(MaxNLocator(integer=False, nbins=4))
...
fig.suptitle("coupling rate and linewidth vs electron nr")
plt.tight_layout()
plt.show()