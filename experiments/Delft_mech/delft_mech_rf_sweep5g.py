...
prefix_name = 'delft_mech_40mVsourcedrive_maxsit'#
postfix = '700mK'
...
gate_amplitude_param = zurich.sigouts.sigouts0.amplitudes.amplitudes1.value
...
gate_amplitude_instrumentlevel = 40e-3
...
outer_gates_sweep=outer_gates[0].sweep(start=start_vgo, stop=stop_vgo, num = step_vgo_num)
outer_gate1_list=list(outer_gates_sweep)
...
meas.register_parameter(outer_gates_sweep.parameter)
meas.register_parameter(freq_sweep.parameter)
#meas.register_parameter(inner_gate_sweep.parameter)   # 
meas.register_custom_parameter('V_rf', 'Amplitude', unit='V', basis=[], setpoints=[outer_gates_sweep.parameter,freq_sweep.parameter])
meas.register_custom_parameter('Phase', 'Phase', unit='rad', basis=[], setpoints=[outer_gates_sweep.parameter,freq_sweep.parameter])
meas.register_custom_parameter('I_rf', 'current', unit='I', basis=[], setpoints=[outer_gates_sweep.parameter,freq_sweep.parameter])
...
meas_aux.register_parameter(outer_gates_sweep.parameter)  # 
meas_aux.register_parameter(cs_sweep.parameter)
meas_aux.register_custom_parameter('G', 'G', unit='S', basis=[], setpoints=[outer_gates_sweep.parameter,cs_sweep.parameter])
meas_aux.register_custom_parameter('V_aux', 'Amplitude_aux', unit='V', basis=[], setpoints=[outer_gates_sweep.parameter,cs_sweep.parameter])
meas_aux.register_custom_parameter('Phase_aux', 'Phase_aux', unit='rad', basis=[], setpoints=[outer_gates_sweep.parameter,cs_sweep.parameter])
meas_aux.register_custom_parameter('approx_sitpos', 'approx_sitpos', unit='V', basis=[], setpoints=[outer_gates_sweep.parameter,cs_sweep.parameter])
...
with meas.run() as datasaver:
    ...
    with meas_aux.run() as datasaver_aux:
        ...
        for outer_gate_value in tqdm(outer_gates_sweep, leave=False, desc='outer Gate Sweep', colour = 'green'): #slow axis loop (gate)
            #print('temperature')
            #Triton.MC()
            for gate in outer_gates:
                gate(outer_gate_value)
            ...
            datasaver_aux.add_result(('G', Glist),
                            ...
                            (outer_gates_sweep.parameter,outer_gate_value),
