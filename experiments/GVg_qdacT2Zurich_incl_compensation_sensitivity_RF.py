...
from utils.CS_utils import zurich_phase_voltage_current_conductance_compensate, zurich_phase_voltage_current_conductance, save_metadata_var, get_var_name
...
amplitude = 100e-6  # 
frequency = 1e3    # Hz
demod_sens_bw=100 #Hz

print(f"source amp at CNT for GVg:{vsdac*1e6} uV")
device_name = 'CD11_D7_C1'
prefix_name = 'sensitivity_self_cs_100bw_filter_8th_order_10uVpkpk'
#postfix = '25mK_'
# exp_name = 'Test 50 K'
...
vars_to_save=[demod_sens_bw,amplitude,frequency,gate_ramp_slope,tc,vsd_dB,source_amplitude_instrumentlevel_GVg,vsdac,x_avg,y_avg]
...
sensitivity_demod_param = zurich.demods.demods4.sample
...
sine_wave_context = gate.sine_wave(
...
meas.register_custom_parameter('G_sens', 'G_sens', unit='S', basis=[], setpoints=[vgdc_sweep.parameter])
meas.register_custom_parameter('V_r_sens', 'Amplitude_sens', unit='V', basis=[], setpoints=[vgdc_sweep.parameter])
meas.register_custom_parameter('Phase_sens', 'Phase_sens', unit='rad', basis=[], setpoints=[vgdc_sweep.parameter])
meas.register_custom_parameter('I_sens', 'Current_sens', unit='A', basis=[], setpoints=[vgdc_sweep.parameter])
...
with meas.run() as datasaver:
    ...
    for vgdc_value in tqdm(vgdc_sweep, leave=False, desc='gate voltage Sweep', colour = 'green'):
        ...
        sensitivity_demod_value=sensitivity_demod_param() 
        theta_calc, v_r_calc, I,  G = zurich_phase_voltage_current_conductance_compensate(measured_value, vsdac,x_avg, y_avg)
        R = 1/G
        theta_calc_sens, v_r_calc_sens, I_sens,  G_sens = zurich_phase_voltage_current_conductance(sensitivity_demod_value, vsdac)
        datasaver.add_result(('R', R),
                            ...
                            ('I', I),
                            ('G_sens', G_sens),
                            ('V_r_sens', v_r_calc_sens),
                            ('Phase_sens', theta_calc_sens),
                            ('I_sens', I_sens),
