...
instr_power_sweep=[1e-6,2e-6,3e-6,4e-6,5e-6,6e-6,7e-6,8e-6,9e-6,10e-6]
...
#####################
stop_f = 121.88e6#122e6 #Hz unit
start_f = 121.96e6# 121.94e6 #Hz unit
step_num_f = 70*10#1 #50Hz
...
meas.register_custom_parameter('time_', 'time_', unit='t', basis=[], setpoints=[drive_mag_param,freq_sweep.parameter])
    ...
    with meas_aux.run() as datasaver_aux:
        ...
        #for instr_magVrms in instr_power_sweep:
            ...
            if reversed_sweep==False:
                ...
                gate_amplitude_CNT=d2v(v2d(np.sqrt(1/2)*instr_magVrms)-att_gate_dB)
