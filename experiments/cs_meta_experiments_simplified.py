...
class CS_meta(CSExperiment):
    ...
    def measure_singledot_config(self,
        ...
        _,I_sens_sit=self.sit_at_max_Isens(side="right")
        print("FINDING MECHANICAL MODE")
        f_max,_=self.find_mech_mode(start_drive=75e-3,end_drive=200e-6,freq_range=None,found_range=4.5e-12,start_step_pitch=0.5e3,div_factor=4,div_f=2,min_sig_I=4e-12,avg_num=3)
        if f_max==None:
             ("MOVING TO NEXT POS")
             return
    ...
    def go_through_gate_pos(self,pos_list=pos_list1,
                            ...
                            background_reps=10,
                            therm_reps=5,temp_meas_counts=2,
                            softening_pitch=2e-4,softening_reps=3):
        for i, pos in enumerate(pos_list):
            ...
            if i % 5 == 0:
                 ...
                 background_id=run_thermomech_temp_meas(reps_nodrive=background_reps,take_time_resolved_spectrum=True,background_id=None)
            else:
                 softening=False
            if i==0:
                 softening=False
