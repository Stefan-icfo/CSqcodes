...
class CS_meta(CSExperiment):
    ...
    def load_parameters(self):
        ...
        self.pos_list_5g = params.pos_list_5g

    def therm_vs_sitpos(self, f_mech, reps_nodrive=10, softening_pitch=0.5e-4, demod_only=False):
        # maybe add separate nr of reps for background here...
        ...
        start_vg = approx_maxpos - 1e-3
        stop_vg = approx_maxpos + 1e-3
        mech_freq = f_mech
        zurich.set_mixdown(mech_freq - 1e6)
        qdac.ch06.dc_constant_V(start_vg)
        background_id = run_thermomech_temp_meas(
            exp_name=f'background_',
            reps_nodrive=reps_nodrive,
            background_id=None
        )  # and here...and in calling fkt
        time.sleep(100)
        i = 0
        if not demod_only:
            while qdac.ch06.dc_constant_V() < stop_vg:
                ...
                run_thermomech_temp_meas(
                    exp_name=f'thermalV_gcs_={current_V*1e3:6g} mV',
                    reps_nodrive=reps_nodrive,
    ...
    def measure_singledot_config(
        ...
        f_max, _ = self.find_mech_mode(
            start_drive=10e-3,
            end_drive=100e-6,
            freq_range=find_freq_range,
            found_range=self.findM_found_range,
            start_step_pitch=1e6,
            div_factor=4,
            div_f=2,
            min_sig_I=1.5e-12,
            min_initial_sig_I=1.9e-12,
            avg_num=None  # 1
        )
        if f_max is None:
        ...
        if driven_traces:
            for n in range(driven_avg_num_meta):
                self.mech_simple_fun_db(
                    costum_prefix="for_avg_singledot_config_" + name_addition,
                    start_f=updated_freq - driven_range_meta / 2,
                    stop_f=updated_freq + driven_range_meta / 2,
                    step_num_f=abs(round(driven_range_meta / driven_pitch_meta))
                )
        zurich.output1_amp1(0)
        if thermal_spectra:
            print("THERMOMECHANICAL SPECTRUM")
            for n in range(temp_meas_counts):
                updated_freq = run_thermomech_temp_meas(
                    reps_nodrive=therm_reps,
        ...
        if thermal_softening:
            ...
            self.therm_vs_sitpos(
                f_mech=f_max,
                reps_nodrive=softening_reps,
                softening_pitch=softening_pitch,
                demod_only=softening_demod_only
            )
        return updated_freq

    def go_through_gate_pos(
        self,
        pos_list=None,
        name_addition=None,
        gate=qdac.ch02,
        ...
        for i, pos in enumerate(pos_list):
            ...
            if i % 5 == 0:
                ...
                if self.manual_background_set is None:
                    ...
                    time.sleep(100)
                    background_id = run_thermomech_temp_meas(
                        exp_name=f"backgroundspecat_{pos}",
                        reps_nodrive=background_reps,
                        take_time_resolved_spectrum=True,
                        background_id=None
                    )
    ...
    def go_through_gate_pos_full_coordinates(
        self,
        pos_list_5g=None,
        name_addition=None,
    ):
        self.load_parameters()
        if pos_list_5g is None:
            pos_list = self.pos_list_5g
        else:
            pos_list = pos_list_5g
        for i, pos in enumerate(pos_list):
            self.load_parameters()
            if name_addition is None:
                name_addition_full = f"step_{i+1}"
            ...
            print(f"ramping to next step nr {i+1} at")
            qdac.read_channels()
            time.sleep(10)
            qdac.ramp_multi_ch_slowly(
                [1, 2, 3, 4, 5],
            ...
            for freq_band in freq_bands:
                ...
                self.measure_singledot_config(
                    ...
                    softening_reps=self.softening_reps,
                    softening_pitch=self.softening_pitch,  ##########
                    background_id=background_id,  # self.manual_background_set,
                    name_addition=name_addition_full,
                    softening_demod_only=False
                )  # for now only demod
                # general
    ...
    def ramp_to_gate_pos(
        ...
        increment=None,
        startpos_gate=None,
        startpos_auxgate=None
    ):
        self.load_parameters()
        if increment is None:
            increment = self.increment_meta
        if startpos_gate is None:
            startpos_gate = self.startpos_gate_meta
        if startpos_auxgate is None:
    ...
    def go_through_gate_pos_softening_only(
        self,
        pos_list=None,
        name_addition=None,
        gate=qdac.ch02,
    ...
    def go_along_line(
        ...
        for gateV, auxgateV in zip(gate_pos, auxgate_pos):
            ...
            if i % 5 == 0:
                ...
                background_id = run_thermomech_temp_meas(
                    exp_name=f"backgroundspecat_{gateV}",
                    reps_nodrive=background_reps,
                    take_time_resolved_spectrum=True,
                    background_id=None
                )
    ...
    #############################
    def repeat_linesweep(
        self,
        run_id,
        gate_nr=2,
        auxgate_nr=1,
        step_nr=100,
        adjust_constant_gates=False
    ):
        ...
        if adjust_constant_gates:
            ...
            for constant_gate_nr in constant_gate_nrs:
                start_voltages.append(
                    metadata[f'qdac_ch0{constant_gate_nr}_dc_constant_V']
                )
            qdac.ramp_multi_ch_slowly(
                constant_gate_nrs,
                start_voltages,
        ...
        self.linesweep_parallel_LFsens_extended(
            costum_prefix=f'rep_{run_id}',
