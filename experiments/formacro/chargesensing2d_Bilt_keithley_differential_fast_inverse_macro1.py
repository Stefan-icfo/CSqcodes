...
prefix_name = '_k2400_chargesensing_postanneal_inverse'
#prefix_name = 'test'
postfix = 'no_postfix'
offset = 80e-6 #voltage offset of k2400
offset_i=-40e-12
...
#swept contacts
outer_gate=bilt.ch03.v  # swept gate voltage

inner_gate=bilt.ch01.v
inner_auxgate1=bilt.ch04.v
inner_auxgate2=bilt.ch02.v
...
inner_auxgate1(start_vgi)
inner_auxgate2(start_vgi)
inner_gate(start_vgi)
print('wait time')
#time.sleep(10)
sleeptime=max(abs(start_vgo-outer_gate()),abs(start_vgi-inner_auxgate1()),abs(start_vgi-inner_auxgate2()),abs(start_vgi-inner_gate()))/gate_ramp_speed/1000+20
print(sleeptime)
time.sleep(sleeptime)
print("wake up, gates are")
print(outer_gate())
print(inner_auxgate1())
print(inner_auxgate2())
print(inner_gate())
...
with meas.run() as datasaver:
    ...
    for outer_gate_value in tqdm(outer_gate_sweep, leave=False, desc='outer Gate Sweep', colour = 'green'): #slow axis loop (gate)
        ...
        for inner_gate_value in tqdm(inner_gate_sweep, leave=False, desc='inner gate Sweep', colour = 'blue'): #fast axis loop (source) #TD: REVERSE DIRECTION
            ...
            inner_auxgate1(inner_gate_value)
            inner_auxgate2(inner_gate_value)
