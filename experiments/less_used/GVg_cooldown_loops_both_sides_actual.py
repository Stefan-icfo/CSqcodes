...
#IN CONSTRUCTION
#this code is in this form not useful as the hemt cant be contiuously switched on during cooldown
...
from drivers.Qdac_utils import ramp_time,read_channels,ramp_to_zero
...
vsd_dB = 0# zero for test 39 # attenuation at the source in dB  ###############SET BACK TO -39!!!!
...
temp_setpoints=[22,20.5,20.4,20.3]  ###########SET FOR FIRST TEST AND ACTUAL VALUES
continuous_meas_at_low_T=True
#below that continuous measurement
irrelevant_difference=5e-3#change value below which no more extra measurement is needed  ####NOT USED RN
...
#initial ramp
print(f"sleep for {ramp_time(all_gates,setpoints)+30} seconds to ramp the gates")
time.sleep(ramp_time(all_gates,setpoints)+30)
           ...
           #start by ramping gates to start value, others to zero 
            ...
            for vgrf_value in tqdm(vgrf_sweep, leave=False, desc='csgate voltage Sweep', colour = 'green'):
                ...
                datasaver1.add_result(('R', R),
                                    ...
                                    ('V_r', v_r_calc),
                                    ('Phase', theta_calc),
                                    (Vgrf_param ,vgrf_value),(Temp_param,current_temp))
            ...
            #linear regression fit
                ...
                #pt.show()
                ###################for test replaced current T with setpoint
                datasaver2.add_result(('Conductance_IV', G_IV),('Resistance_IV', R_IV),('Current_top', I_top),('Current_zero', I_zero),('Current_bottom', I_bottom),(Vgdc_param, vgdc_value),(Temp_param,current_temp))
