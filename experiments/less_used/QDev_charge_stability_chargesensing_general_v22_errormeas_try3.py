...
#####################
start_vg1 = -1.979#-1.934#
stop_vg1 = -1.978#-1.929 #delta 15
step_vg1_num =1*5#500uv
...
#####################
start_vg2 = -1.943#-1.885 #
stop_vg2 =  -1.933#-1.875#delta=10mV
step_vg2_num=10#250uV
step_vg2=np.absolute((start_vg2-stop_vg2)/step_vg2_num)
vars_to_save.append(step_vg2)

start_vgcs=-1.515#0.0372 #-0lowerV slope, 140nS
#GVg params
...
meas.register_custom_parameter('peak_position_alt', 'V_peak_alt', unit='Volt', basis=[], setpoints=[gate1_sweep.parameter,gate2_sweep.parameter])
meas.register_custom_parameter('error_V', 'V_error', unit='Volt', basis=[], setpoints=[gate1_sweep.parameter,gate2_sweep.parameter])
meas.register_custom_parameter('peak_G_alt', 'peak_G_alt', unit='Volt', basis=[], setpoints=[gate1_sweep.parameter,gate2_sweep.parameter])
meas.register_custom_parameter('Gamma_alt', 'Gamma_alt', unit='Volt', basis=[], setpoints=[gate1_sweep.parameter,gate2_sweep.parameter])
meas.register_custom_parameter('fit_error_alt', 'fit_error_alt', unit='', basis=[], setpoints=[gate1_sweep.parameter,gate2_sweep.parameter])
...
with meas.run() as datasaver:
   ...
   # datasaver.dataset.add_metadata('qdac_ch06_dc_constant_V',qdac.ch06.dc_constant_V())
    ...
    for gate1_value in tqdm(gate1_sweep, leave=False, desc='outer gate sweep', colour = 'green'): #slow axis loop (gate)
        ...
        #####################################################################
        alt_peakpos_list=[]#for error measurement; peakpos measured by second streak
        error_list=[]#for error measurement; difference between actual and estimated value
        alt_peakG_list=[]
        alt_peakGamma_list=[]
        alt_fiterror_list=[]
        #####################################################################
        ...
        for gate2_value in tqdm(gate2_sweep, leave=False, desc='inner gate sweep', colour = 'blue'): #fast axis loop (source) #TD: REVERSE DIRECTION
            ...
            ###now do the GVg and later decide if use it for both or only for one

            ############################################################################
             ### now start of GVg loop to be moved earlier
            ...
            peak_fit_alt, hgamma_fit_alt, peak_G_fit_alt=popt

            alt_peakpos_list.append(peak_fit_alt)
            alt_peakG_list.append(peak_G_fit_alt)
            alt_peakGamma_list.append(hgamma_fit_alt)
            alt_fiterror_list.append(pcov)


                 ### now end of GVg loop to be moved earlier
                #########################################################################
            ...
            if First_run==False:
                ...
                print(f"peak_fit_no_GVg={peak_fit}")
            ...
            if G_min<G<G_max and First_run==False and do_GVg_anyway==False and GinRange and G>upper_noise_bound: #no GVg
                ...
                ###############################################
                 #error measurement
                error_list.append(peak_fit_alt-peak_fit)
                print(f"difference:{peak_fit_alt-peak_fit}")
                ###############################################
                First_run=False
                ...
                #doing GVg
                print("using GVg")
                peak_fit, hgamma_fit, peak_G_fit=popt #use GVg values also for normal streak
               
               #################################################
                 #error measurement, here same as peak_fit
                error_list.append(0)
                #################################################
                ...
                #here step in again from GVG, calculate detuning etc
        ...
        if reversed_sweep: #if the sweep is reversed then the measurement lists have to be reversed too, since fast_axis_unreversible_list has the values for the unreversed sweep. double-check on a measurement if it really works as intended!
            ...
            ###############################################
            alt_peakpos_list.reverse()
            error_list.reverse()

            alt_peakG_list.reverse()
            alt_peakGamma_list.reverse()
            alt_fiterror_list.reverse()###still gotta define and add!!!!
            ###########################################
        ...
        datasaver.add_result(('G', Glist),
                          ...
                          #  ('peak_position',peakpos_list),
                            ...
                            ##################################################################################
                            ('peak_position_alt',alt_peakpos_list),
                            ('error_V',error_list),
                            ('peak_G_alt',alt_peakG_list),
                            ('Gamma_alt',alt_peakGamma_list),
                            #('fit_error_alt',alt_fiterror_list),
                            #################################################################################
