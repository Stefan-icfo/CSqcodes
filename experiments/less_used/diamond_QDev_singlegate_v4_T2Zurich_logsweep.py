...
prefix_name = 'Diamond_logsweep_withg1-5at+0.x-2+1.1-2+0.x_repeat'
postfix = '___'
# exp_name = 'Test 50 K'
...
logcoeff_max=np.log10(max_vs)
logcoeff_min=np.log10(min_vs)
#source_logsweep=[]
source_logsweep=list(-np.logspace(start=logcoeff_min, stop=logcoeff_max, num=log_step_vs_num_each))
source_logsweep.reverse()
source_logsweep.append(0)
source_logsweep.extend(list(np.logspace(start=logcoeff_min, stop=logcoeff_max, num=log_step_vs_num_each)))#in mV, positive part
...
source.dc_constant_V(-max_vs)



time.sleep(max([abs((start_vg-gate.dc_constant_V())/ramp_speed_gate),abs((-max_vs-source.dc_constant_V())/ramp_speed_source)])+1)  #wait for the time it takes to do both ramps plus one second

last_source_value=source.dc_constant_V()

source_logsweep_param = Parameter('source_logsweep_param',
                            label='Source Logarithmic Sweep',
...
source_logsweep_param.set(last_source_value)
...
meas.register_parameter(source_logsweep_param)  # 
meas.register_custom_parameter('R', 'R', unit='Ohm', basis=[], setpoints=[gate_sweep.parameter,source_logsweep_param])
meas.register_custom_parameter('V_r', 'Amplitude', unit='V', basis=[], setpoints=[gate_sweep.parameter,source_logsweep_param])
meas.register_custom_parameter('Phase', 'Phase', unit='rad', basis=[], setpoints=[gate_sweep.parameter,source_logsweep_param])
meas.register_custom_parameter('G', 'G', unit='S', basis=[], setpoints=[gate_sweep.parameter,source_logsweep_param])
...
with meas.run() as datasaver:
    ...
    fast_axis_unreversible_list = list(source_logsweep) #(to deal with snake)
    ...
    for gate_value in tqdm(gate_sweep, leave=False, desc='Gate Sweep', colour = 'green'): #slow axis loop (gate)
        ...
        for source_value in source_logsweep:
            source.dc_constant_V(source_value)
            source_logsweep_param.set(source_value)
        ...
        datasaver.add_result(('R',Rlist),
                            ...
                            (source_logsweep_param,fast_axis_unreversible_list))
        source_logsweep.reverse() 
