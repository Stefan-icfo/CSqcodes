# standard charge sensing of stability diagram with Dave-method (second measurement)
...
from utils.CS_utils import breit_wigner_fkt, zurich_phase_voltage_current_conductance_compensate, breit_wigner_detuning
#------User input----------------
...
#####################
#start_vg2 = -1.970  #
#stop_vg2 = -1.964#delta=10mV
#step_vg2_num =  6*50+1# 20uV
...
#outer voltage range (slow axis)
#####################
start_vg1 =0.546#-2.482#-1.934#
stop_vg1 = 0.555#-2.477#-1.929 #delta 15
step_vg1_num =9*5#10uv
step_vg1=np.absolute((start_vg1-stop_vg1)/step_vg1_num)
...
#inner voltage range (fast axis)
#####################
start_vg2 = 0.782#-1.5785#
stop_vg2 = 0.794#-1.5735#
#stop_vg2 =  -1.571#-1.875#delta=10mV
step_vg2_num=12*10#5uV
step_vg2=np.absolute((start_vg2-stop_vg2)/step_vg2_num)
vars_to_save.append(step_vg2)


start_vgcs=0.84#0.0372 #-0lowerV slope, 140nS
#GVg params
#GVg params
...
with meas.run() as datasaver:
    ...
    for gate1_value in tqdm(gate1_sweep, leave=False, desc='outer gate sweep', colour = 'green'): #slow axis loop (gate)
        ...
        for gate2_value in tqdm(gate2_sweep, leave=False, desc='inner gate sweep', colour = 'blue'): #fast axis loop (source) #TD: REVERSE DIRECTION
            ...
            G_max_corrected=peak_G_fit*sitfraction*(1+tolerance)
            G_min_corrected=peak_G_fit*sitfraction*(1-tolerance)
            ...
            if First_run==False:
                ...
                if abs(fit_position_correction)>5e-6:#nearly always
                    ...
                    time.sleep(tc)
                    theta_calc, v_r_calc, I, G = zurich_phase_voltage_current_conductance_compensate(measured_value, vsdac,x_avg,y_avg)#confirmation 

            if G_min_corrected<G<G_max_corrected and First_run==False and do_GVg_anyway==False and GinRange and G>upper_noise_bound: #no GVg
                ...
                if debug:
                    print(f"doing GVg because either Gmincorr={G_min_corrected}, G={G}, Gmaxcorr={G_max_corrected}, Ginrange={GinRange}, peakGfit={peak_G_fit}, uppernoisebound={upper_noise_bound}")
                #print("GVg")
                if First_run==False:
                    last_sitpos=copy.copy(current_csvg)
                nr_GVgs+=1
                if G>=G_max_corrected:
                    nr_of_topslips+=1
                elif G<=G_min_corrected:
                    nr_of_bottomslips+=1
                else:
                    
                    if debug and do_GVg_anyway==False:
                        plt.figure(10)
                ...
                if debug:
                    ...
                    plt.plot(csweeplist,Glistcs)
                    plt.plot(fit_calculated_sitpos,G_max_corrected,'bo')
                    plt.plot(fit_calculated_sitpos,G_min_corrected,'bo')
