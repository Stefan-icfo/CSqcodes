#done on 291025. does masks in canse of noise
...
#    zurich.set_mixdown(mode_freq)
    ...
    experiment_1D = new_experiment(name=exp_name+'_1D'+f"g2_at_{round(qdac.ch02.dc_constant_V(),4)}_outputfreq={round(zurich.freq1(),4)}", sample_name=device_name)
    ...
    with meas.run() as datasaver:
        ...
        with meas_aux_aux.run() as datasaver_aux_aux:
            ...
            masked_compressed_freq_array=compressed_freq_array[mask]
            ...
            if background_id is None:
              ...
              #                          ('avg_avg_psd_drive',avg_avg_driven_psd[mask]),
                ...
                masked_integral_over_substracted_psd=np.sum(avg_avg_psd_nodrive_avg_substracted[mask])
        ...
        if background_id is not None:
            ...
            datasaver.dataset.add_metadata('masked_integral_over_substracted_psd',masked_integral_over_substracted_psd)
            print(f"integral_over_substracted_psd {integral_over_substracted_psd}")
            return masked_compressed_freq_array[np.argmax(centered_moving_average(avg_avg_psd_nodrive_avg_substracted[mask],n=maxfind_avg_avg_num))]
        else:
            return run_id_1D
